...
- Commit에서 부족한 부분
   - 사실 브랜치를 이용해 서로 다른 곳에서 작업을 해본 것이 처음이 었다.
     그래서 github_Desktop에서 propt을 이용한 main 브랜치의 프로젝트 정보 갱신이 잘 안이루어 졌던 것 같다.
     그러다 보니 이전의 commit된 정보로 갱신되어 코드가 뒤바뀌어 있던 적이 여러번 있었던 것 같다.
     추후에는 merge후 바로바로 propt을 이용해 갱신 했지만 다음번에 이 작업이 꼭 되어야 할 것 같다.
